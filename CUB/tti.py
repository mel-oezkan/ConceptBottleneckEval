...
def get_stage2_pred(a_hat):
    ...
    stage2_inputs = (
        torch.from_numpy(np.array(a_hat)).to("cuda" if is_cuda else "cpu").float()
    )
    ...
    for _ in range(n_trials):
        ...
        if mode == "random":
            ...
            replace_fn = (
                ...
                img_id,
                n_replace,
                replace_cached: replace_entropy_adaptive(
                    attr_preds,
                    attr_preds_sigmoid,
                    attr_labels,
                    img_id,
        ...
        stage2_inputs = torch.from_numpy(np.array(b_attr_new)).to(device=device)
        if connect_CY:  # class_outputs is currently contributed by C --> Y
            ...
            class_outputs = torch.from_numpy(b_class_logits).to(device=device) + (
