...
def get_few_shot_data(n_samples: int, out_dir: str, data_file="train.pkl") -> None:
    ...
    Args:
        n_samples (int): How many samples to select per class
        out_dir (str): Ouput directory
        data_file (str, optional): Processed data file. Defaults to "train.pkl".
    """
    random.seed(0)
    data = pickle.load(open(data_file, "rb"))
    class_to_data = ddict(list)

    # add all data-points to the corresponding class list
    for d in data:
        class_to_data[d["class_label"]].append(d)

    # create the few-shot data
    new_data = []
    for c, data_list in class_to_data.items():
    # iterate over class_label: c, file_information: data_list
        if len(data_list) < n_samples:
            print(
                "Class %d does not have enough samples. " 
                "Add all of %d instances to the new dataset"
                % (c, len(data_list))
            )
...
def get_class_attributes_data(
    ...
    mask = np.where(
        attr_class_count >= min_class_count
    )[
        0
    ...
    create_new_dataset(
        out_dir, "attribute_label", collapse_fn, data_dir=modify_data_dir
    )
...
def shuffle_class(out_dir, data_dir):
    ...
    shuffle_fn = lambda d: class_to_attr_setting[
        new_to_old_label[d["class_label"]]
    ]  # change attribute labels instead of class labels
...
def split_into_groups(single_attr, attr_name_file):
    ...
    if not single_attr:  # attribute group
        ...
        start_indices = list(range(N_ATTRIBUTES))[
            :10
        ]  # generate data for the first 10 individual attributes to experiment
...
def change_img_dir_data(
    new_image_folder, datasets, data_dir="", out_dir="masked_datasets/"
):
    """
    Change the prefix of img_path data in data_dir to new_image_folder
    """
    compute_fn = lambda d: os.path.join(
        new_image_folder, d["img_path"].split("/")[-2], d["img_path"].split("/")[-1]
    )
    create_new_dataset(
        out_dir, "img_path", datasets=datasets, compute_fn=compute_fn, data_dir=data_dir
    )


def create_logits_data(
    model_path, out_dir, data_dir="", use_relu=False, use_sigmoid=False
):
    """
    ...
    get_logits_train = lambda d: inference(
        d["img_path"], model, use_relu, use_sigmoid, is_train=True
    )
    get_logits_test = lambda d: inference(
        d["img_path"], model, use_relu, use_sigmoid, is_train=False
    )
    create_new_dataset(
        out_dir,
        "attribute_label",
...
def get_representation_linear_probe(model_path, layer_idx, out_dir, data_dir):
    model = torch.load(model_path)
    get_representation_train = lambda d: inference_no_grad(
        d["img_path"],
        model,
        use_relu=False,
        use_sigmoid=False,
    ...
    get_representation_test = lambda d: inference_no_grad(
        d["img_path"],
        model,
        use_relu=False,
        use_sigmoid=False,
...
def inference(
    img_path, model, use_relu, use_sigmoid, is_train, resol=299, layer_idx=None
):
    """
    ...
    if layer_idx is not None:
        ...
        cropped_model = torch.nn.Sequential(
            *list(model.children())[:layer_idx]
        )  # nn.ModuleList(all_mods[:layer_idx])
...
def inference_no_grad(
    ...
    """
    with torch.no_grad():
        attr_outputs = inference(
            img_path, model, use_relu, use_sigmoid, is_train, resol, layer_idx
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[
            "ExtractConcepts",
            "ExtractProbeRepresentations",
            "DataEfficiencySplits",
    ...
    parser.add_argument(
        "--train_splits", type=str, nargs="+", help="Train splits to use"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Layer id to extract probe representations",
    )
    parser.add_argument(
        "--n_samples", type=int, help="Number of samples for data efficiency split"
    )
    ...
    if args.exp == "ExtractConcepts":
        ...
        get_representation_linear_probe(
            args.model_path, args.layer_idx, args.out_dir, args.data_dir
        )
    elif args.exp == "ChangeAdversarialDataDir":
        change_img_dir_data(
            args.adv_data_dir,
            datasets=args.train_splits,
            data_dir=args.data_dir,
            out_dir=args.out_dir,
        )
    elif args.exp == "DataEfficiencySplits":
        get_few_shot_data(
            args.n_samples, args.out_dir, os.path.join(args.splits_dir, "train.pkl")
        )
        get_few_shot_data(
            args.n_samples, args.out_dir, os.path.join(args.splits_dir, "val.pkl")
        )
        copyfile(
            os.path.join(args.splits_dir, "test.pkl"),
            os.path.join(args.out_dir, "test.pkl"),
        )
